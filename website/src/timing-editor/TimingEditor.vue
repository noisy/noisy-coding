...
import { CONSOLE_ACTIVITIES, userTurns, validatePresentation, type ActivityKind, type PresentationEdits } from '@dashboard/components/marketing/presentationTiming';
...
const workWindows = computed(() => edits.value.turns.filter(edit => edit.status !== 'none').map(edit => ({...edit, end: turns.find(turn => turn.utterance === edit.utterance)!.replyMs!})));
function saveDraft() { try { localStorage.setItem(cacheKey, JSON.stringify(edits.value)); } catch { /* JSON download remains available. */ } }
function update(end = edit.value.userEndMs, status = edit.value.status) {
  try {
    const next = {...edits.value, turns: [...edits.value.turns.filter(edit => edit.utterance !== selected.value), {utterance: selected.value, userEndMs: end, status}]};
...
function resetTurn() { edits.value = {...edits.value, turns: edits.value.turns.filter(edit => edit.utterance !== selected.value)}; dirty.value = true; saveDraft(); error.value = ''; }
function download() {
...
<template>
  <main>
    ...
    <div class="workspace">
      <div ref="frame" class="preview" :style="{height: `${760 * scale}px`}"><RecordedHeroScene ref="scene" manual-playback :presentation-edits="edits.turns" :style="{transform: `scale(${scale})`, transformOrigin: 'top left'}" @time="onTime" /></div>
    ...
    <section class="timeline" aria-label="Presentation timeline">
      <div class="track"><span class="track-label">You</span><button v-for="(turn, i) in turns" :key="turn.utterance" class="user segment" :class="{selected: selected === turn.utterance}" :style="position(turn.startMs, turn.endMs)" :aria-label="`Select user turn ${i+1}`" @click="selected = turn.utterance"><i :style="{width: `${(turnEnd(turn.utterance, turn.endMs) - turn.startMs) / (turn.endMs - turn.startMs) * 100}%`}" /><b>{{ i + 1 }}</b></button></div>
      <div class="track"><span class="track-label">Work</span><button v-for="work in workWindows" :key="work.utterance" class="work segment" :style="position(work.userEndMs, work.end)" :title="work.status" @click="seek(work.userEndMs)">{{ work.status }}</button></div>
      <div class="track"><span class="track-label">Lux</span><button v-for="reply in replies" :key="reply.start" class="agent segment" :style="position(reply.start, reply.end)" :title="reply.text" @click="seek(reply.start)">Lux</button></div>
