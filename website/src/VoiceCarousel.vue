...
const playing = ref('');
const error = ref('');
const SAMPLE_PLAYBACK_RATE = 1.2;
...
async function toggleSample(name: string) {
  ...
  audio = new Audio(src);
  audio.playbackRate = SAMPLE_PLAYBACK_RATE;
...
<template>
  <div class="voice-carousel" role="region" aria-label="Explore agent voices">
    <div ref="track" class="voice-track" :class="{ 'more-right': !atEnd }" @scroll="update">
      <button v-for="voice in voices" :key="voice.name" class="voice-card" type="button"
        :disabled="!samples[`./assets/voice-intros/${voice.name.toLowerCase()}.mp3`]"
        :aria-label="`${playing === voice.name ? 'Stop' : 'Listen to'} ${voice.name}`"
        :aria-pressed="playing === voice.name" @click="toggleSample(voice.name)">
        <div class="portrait-only">
          ...
          <span class="playback-state" aria-hidden="true">{{ playing === voice.name ? '■ Playing' : '▶ Play voice' }}</span>
        </div>
      </button>
    </div>
...
.voice-card { padding:0; font:inherit; cursor:pointer; }
.voice-card:hover:not(:disabled), .voice-card[aria-pressed="true"] { border-color:var(--amber); background:var(--accent-surface); }
.voice-card:focus-visible { outline:2px solid var(--amber); outline-offset:3px; }
.voice-card:disabled { opacity:.5; cursor:default; }
...
.playback-state { margin-top:10px; color:var(--amber); font-size:11px; }
</style>
