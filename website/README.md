...
`CrewSection.vue` presents `RecordedCrewScene.vue`: the real Companion driven by a continuous webcam recording and exported transcript arrivals. The webcam defaults to a central 50% crop (adjustable through Camera zoom in Storybook), anchored to the screenshot’s bottom-left corner. The user microphone and five existing agent clips share one video clock; transcript offsets affect text only. The original timer-driven `CompanionCrewScene.vue` remains available in Storybook. Portrait cards can be browsed with arrows or swipes. Use Hear the conversation for the scripted voice sequence. Audio starts muted. Click the translucent muted-speaker button on the webcam (or Hear the conversation) to enable sound. Hover does not affect audio; leaving the viewport pauses and mutes it. Mobile uses a tighter static camera; desktop retains the original handover zooms. The recordings ship as Vite assets and need no runtime API key. This is explicitly a scripted example, not a live microphone session.
