...
|---|---|
| `src/noisy_studio/**`, `Dockerfile`, `dashboard/**` | **container**: `/noisy-studio:update` once, any production thread (pulls, quotes Highlights, recreates; volume/key survives) |
| `hooks/**`, `commands/**`, `skills/**`, `.claude-plugin/**` (plugin payload) | **plugin**: `claude plugin update noisy-studio@noisy` — ONCE PER PROFILE (work + `CLAUDE_CONFIG_DIR=~/.claude-personal` for personal) |
...
- **Image-only** (daemon/server/dashboard fix): 1 step — `/noisy-studio:update`.
- **Plugin-only** (hooks/commands/skills): 2 steps × profiles — plugin update per profile, reload per session. No container touch.
- **Both halves** (e.g. a fix spanning mcp_exec.sh AND server.py): 3 steps — plugin update per profile → reloads → `/noisy-studio:update`. Say explicitly that ONE half alone leaves the bug alive (see the 2.7.6 lesson: container on 2.7.6 with plugins on 2.7.5 still misroutes).
...
- The marketplace has no build step — the new plugin version is available
  ...
  truth with `curl -s https://raw.githubusercontent.com/noisy/noisy-studio/main/.claude-plugin/plugin.json | grep version`.
...
1. /noisy-studio:update  (any production thread; plugins untouched)
