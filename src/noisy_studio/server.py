...
async def _identity_error(agent_id: str | None) -> str | None:
    ...
    port = getenv(LISTENER_PORT_ENV_VAR, "8765")
    try:
...
def _ensure_daemon() -> None:
    ...
    """
    if getenv("NOISY_STUDIO_NO_AUTOSPAWN"):
        return
    port = int(getenv(LISTENER_PORT_ENV_VAR, "8765"))
...
def main() -> None:
    ...
    if getenv("NOISY_STUDIO_MCP_TRANSPORT", "stdio") == "http":
        mcp.settings.host = getenv("NOISY_STUDIO_MCP_BIND", "127.0.0.1")
        mcp.settings.port = int(getenv("NOISY_STUDIO_MCP_PORT", "8767"))
