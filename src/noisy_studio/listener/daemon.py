...
from noisy_studio.config_dir import CONFIG_DIR, migrate_legacy_config_dir
...
from noisy_studio.listener.tab_audio import start_bridge
...
OUTPUT_DEVICE_ENV_VAR = "NOISY_STUDIO_OUTPUT_DEVICE"
...
def _ptt_barge_in(state: ListenerState) -> bool:
    ...
    from noisy_studio.listener import tab_audio
...
def _poll_credits(state: ListenerState) -> None:
    ...
    management_key = getenv(MANAGEMENT_KEY_ENV_VAR)
    team_id = getenv(TEAM_ID_ENV_VAR)
...
def run(config: VadConfig | None = None) -> None:
    ...
    # grok-voice -> noisy-studio rename so the API key/settings/history survive.
    migrate_legacy_config_dir()
    config = config or VadConfig()
    port = int(getenv(PORT_ENV_VAR, str(DEFAULT_PORT)))

    state = ListenerState()
    state.set_mode(getenv(MODE_ENV_VAR, "live"))
    state.set_language(getenv(STT_LANGUAGE_ENV_VAR, ""))
    state.set_input_device(getenv(INPUT_DEVICE_ENV_VAR, ""))
    state.set_output_device(getenv(OUTPUT_DEVICE_ENV_VAR, ""))
    try:
    ...
    if getenv(MANAGEMENT_KEY_ENV_VAR) and getenv(TEAM_ID_ENV_VAR):
