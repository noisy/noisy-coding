...
from noisy_studio.listener import audio_cache, tab_audio
...
def resolve_options(
    ...
    voice = character.get("voice") or getenv(DEFAULT_VOICE_ENV_VAR, FALLBACK_VOICE)
    language = state.language or getenv(DEFAULT_LANGUAGE_ENV_VAR) or "auto"
...
def _tts_streaming(state: ListenerState, provider: providers.TTSProvider) -> bool:
    ...
    if state.output_device == "browser":
        ...
        return False
    if getenv(TTS_MODE_ENV_VAR, "").lower() == "live":
        return True
