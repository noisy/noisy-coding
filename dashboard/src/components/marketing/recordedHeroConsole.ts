import type { TurnTiming } from './presentationTiming';
...
export function recordedHeroConsoleAt(take: RecordedTake, timeMs: number, edits: TurnTiming[] = []): TerminalLine[] {
  ...
  const workStarts: RecordedTake['events'] = edits.filter(edit => edit.status === 'console').map(edit => ({
    sequence: Number.MAX_SAFE_INTEGER, atMs: edit.userEndMs, type: 'work-start', utterance: edit.utterance,
  }));
  const events = [...take.events, ...workStarts].sort((a, b) => a.atMs - b.atMs || a.sequence - b.sequence);
  const started = (utterance: string) => edits.some(edit => edit.utterance === utterance && edit.status === 'console');
