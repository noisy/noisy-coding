    ...
    <div class="layout">
      ...
      <section class="avatars">
        ...
        <div class="families" role="group" aria-label="Avatar style">
          <button v-for="set in AVATAR_SETS" :key="set.id" :aria-pressed="avatarSet === set.id" @click="selectAvatarSet(set.id)">
            <span class="faces"><VoiceAvatar v-for="voice in ['lux','iris','rex']" :key="voice" :voice="voice" :set="set.id" :size="36" /></span>
            <span>{{ set.name }}</span>
          </button>
        </div>
        <details><summary>Browse all voices</summary><div class="all-voices"><div v-for="voice in AVATAR_VOICES" :key="voice"><VoiceAvatar :voice="voice" :size="44" /><span>{{ voice }}</span></div></div></details>
      </section>
      <aside class="preview" aria-label="Live conversation preview">
        ...
        <div class="conversation">
          ...
          <Bubble side="right" accent="violet" who="Claude" voice="lux" text="Both tests pass. The fix is ready for review." status-kind="off" status-label="" time="" compact />
...
.layout { display:grid; grid-template-columns:minmax(300px,1fr) minmax(320px,1.05fr); gap:0 32px; align-items:start; }
...
details { margin-top:16px; border-top:1px solid var(--line); padding-top:12px; } summary { cursor:pointer; font-size:12px; color:var(--muted); }
.all-voices { display:grid; grid-template-columns:repeat(auto-fill,minmax(64px,1fr)); gap:12px; margin-top:16px; }
.all-voices > div { display:grid; justify-items:center; gap:4px; font-size:11px; color:var(--muted); }
