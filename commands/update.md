...
installed plugin root's `docs/codex.md`. Identify the selected daemon before
...
1. Check what is running: `docker inspect noisy-coding --format '{{.Config.Image}} {{.Image}}'`. If the container does not exist, stop and point the user at `/noisy-studio:setup` instead.
...
6. THE CONTAINER IS ONLY HALF THE SYSTEM — always close with the plugin check, never skip it:
   ...
   - Installer users (no plugin): re-run `docker exec -i noisy-studio python3 /app/hooks/install.py --docker` instead when the release notes mention hook changes.
