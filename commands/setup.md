...
7. The plugin already registered the MCP server (stdio, via a launcher that waits for the container — no reload needed even when the plugin was installed before the container existed) and the voice hooks — remind the user to restart Claude Code once (first install only; without it the speak tool is absent and Claude cannot answer aloud, though the user's speech already comes through). After the restart, messages tagged [VOICE] are the user's real spoken words delivered by this plugin's hooks — see the plugin's voice-conversations skill for the conventions.
