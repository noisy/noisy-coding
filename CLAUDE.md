...
Code hooks and an MCP server. Python backend in `src/noisy_studio/`, Vue
...
1. a second MCP server named `noisy-studio-dev` (stdio, LOCAL scope — one
...
2. project-scoped hook overrides in this repo's `.claude/settings.json`
   pointing at local `hooks/*.py` with `NOISY_STUDIO_LISTENER_PORT=7765`.
