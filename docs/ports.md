...
|------|------|-----|----------|
| 8765 | HTTP API + dashboard | The daemon: transcript queue for the hooks (`/drain`), live activity (`/activity`), status (`/status`), and it serves the dashboard UI | `NOISY_STUDIO_LISTENER_PORT` |
...
| 8767 | MCP endpoint | `speak`, `announce`, `change_voice`, `list_voices` for agents, over streamable HTTP (a plain GET answers `406` — that is healthy; MCP requires `Accept: text/event-stream`) | `NOISY_STUDIO_MCP_PORT` |
